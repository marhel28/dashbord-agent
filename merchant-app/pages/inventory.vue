  ...
  <div class="space-y-6 animate-fade-in max-w-7xl mx-auto py-2">
    ...
    <div v-else-if="error" class="flex items-center justify-center py-16">
      <div class="text-center space-y-4 max-w-sm border border-red-200 dark:border-red-500/20 p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-slate-950/40">
        ...
        <Button variant="default" size="sm" @click="fetchStocks" class="rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-500 dark:hover:bg-emerald-400 dark:text-slate-950 font-semibold shadow-xs">
          Coba Lagi
        </Button>
      </div>
    </div>
    ...
    <Dialog :open="showDeleteConfirm" @update:open="showDeleteConfirm = $event">
      <DialogContent class="sm:max-w-sm" @close="showDeleteConfirm = false">
        <div class="w-12 h-12 mx-auto flex items-center justify-center mb-4 rounded-full bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400">
          <Icon name="lucide:alert-triangle" class="w-6 h-6" />
        </div>
        <DialogHeader>
