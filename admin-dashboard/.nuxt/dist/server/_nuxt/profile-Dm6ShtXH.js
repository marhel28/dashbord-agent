...
import { u as useAuth, a as useColorMode } from "./composables-D9Qffros.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div class="text-center md:text-left flex-1"><h2 class="text-2xl font-black text-[var(--wp-navy)] dark:text-white">${ssrInterpolate(unref(user)?.name)}</h2><p class="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1">Administrator Utama</p><div class="mt-3 flex flex-wrap gap-2 justify-center md:justify-start"><span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold rounded" style="${ssrRenderStyle({ "background": "rgba(212,168,67,0.1)", "color": "var(--wp-gold-dark)" })}">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "heroicons:envelope",
        class: "w-3.5 h-3.5"
      }, null, _parent));
      _push(` ${ssrInterpolate(unref(user)?.email)}</span>`);
      if (unref(user)?.phone_number) {
        ...
        _push(` ${ssrInterpolate(unref(user)?.phone_number)}</span>`);
      } else {
        _push(`<!---->`);
      }
...
//# sourceMappingURL=profile-Dm6ShtXH.js.map
