  ...
  setup(__props) {
    ...
    const colDefs = ref([
      ...
      },
      {
        ...
        cellRenderer: (p) => {
          var _a;
          const isVerified = p.value !== false && ((_a = p.data) == null ? void 0 : _a.is_verified) !== false;
          if (isVerified) {
            return `<div style="display: flex; align-items: center; height: 100%;">
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value && !stats.value.totalMerchants) {
        ...
        _push(`</div></div></div><div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden flex flex-col h-[650px]"><div class="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div class="flex items-center gap-2"><h2 class="text-base font-bold text-slate-900 dark:text-white">Database Pedagang (Expert View)</h2><span class="px-2 py-0.5 text-[11px] font-semibold bg-amber-50 text-amber-700 border border-amber-200 rounded-full dark:bg-amber-950/50 dark:text-amber-300 dark:border-amber-800"> AG Grid Engine </span></div><p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Tampilan data pedagang berkinerja tinggi dengan pencarian &amp; filter instan.</p></div><div class="flex items-center gap-3"><div class="relative">`);
        _push(ssrRenderComponent(_component_Icon, {
          name: "heroicons:magnifying-glass",
          class: "w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2"
        }, null, _parent));
        _push(`<input type="text"${ssrRenderAttr("value", gridSearch.value)} placeholder="Cari di seluruh kolom..." class="pl-9 pr-3 py-2 text-xs border border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-lg w-72 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm"></div>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/merchants",
          class: "px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold transition-colors flex items-center gap-1.5 shadow-xs"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span${_scopeId}>Buka Halaman Pedagang</span>`);
              ...
              return [
                createVNode("span", null, "Buka Halaman Pedagang"),
        ...
        _push(`</div></div><div class="flex-1 w-full h-full p-3 relative bg-slate-50/30 dark:bg-slate-900/40">`);
...
//# sourceMappingURL=index-DbdQzCsa.mjs.map
